...
@property (nonatomic, strong) NSDateFormatter *overlayDateFormatter; //Will be used to format date in header view and on scroll.
...
@property (nonatomic, readonly) PDTSimpleWeekView *weekView;

@property (nonatomic, readonly) NSMutableDictionary *cacheDateForSection;
@property (nonatomic, readonly) NSMutableDictionary *cacheDateForCell;
...
@synthesize cacheDateForSection = _cacheDateForSection;
@synthesize cacheDateForCell = _cacheDateForCell;
...
- (void)simpleCalendarCommonInit
{
    ...
    self.daysPerWeek = 7;
    _cacheDateForSection = [NSMutableDictionary new];
    _cacheDateForCell = [NSMutableDictionary new];
}
...
- (NSDateFormatter *)headerDateFormatter;
{
    if (!_headerDateFormatter) {
        ...
        _headerDateFormatter.dateFormat = [NSDateFormatter dateFormatFromTemplate:@"LLL" options:0 locale:self.calendar.locale];
    }
...
- (NSDateFormatter *)overlayDateFormatter;
{
    if (!_overlayDateFormatter) {
        ...
        _overlayDateFormatter.dateFormat = [NSDateFormatter dateFormatFromTemplate:@"yyyy LLL" options:0 locale:self.calendar.locale];
    }
    return _overlayDateFormatter;
}

- (PDTSimpleWeekView *)weekView{
    if(!_weekView){
        CGFloat width = CGRectGetWidth(self.view.bounds);
        CGFloat height = 44.f;
        UINavigationBar *bar = self.navigationController.navigationBar;
        if (bar) {
            width = CGRectGetWidth(bar.frame);
            height = CGRectGetHeight(bar.frame);
        }
        _weekView = [[PDTSimpleWeekView alloc] initWithFrame:CGRectMake(0, height, width, 20.f) calendar:self.calendar];
    }
    return _weekView;
}
...
{
    _calendar = calendar;
    if([_calendar respondsToSelector:@selector(setFirstWeekday:)]){
        [_calendar setFirstWeekday:1];
    }
    self.headerDateFormatter.calendar = calendar;
    self.overlayDateFormatter.calendar = calendar;
...
- (void)setFirstDate:(NSDate *)firstDate
{
    ...
    _firstDateMonth = nil;
    [_cacheDateForSection removeAllObjects];
    [_cacheDateForCell removeAllObjects];
}
...
- (void)loadView{
    ...
    [self.view addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"V:|-20-[overlayView(==overlayViewHeight)]" options:NSLayoutFormatAlignAllTop metrics:metricsDictionary views:viewsDictionary]];
}
...
- (void)viewWillAppear:(BOOL)animated{
    ...
    if (self.navigationController.navigationBar) {
        if (!_weekView) {
            [self.collectionView setContentInset:UIEdgeInsetsMake(30.f, 0, 0, 0)];
            UIColor *bgColor = self.navigationController.navigationBar.tintColor;
            if ([self.navigationController.navigationBar respondsToSelector:@selector(barTintColor)]) {
                bgColor = self.navigationController.navigationBar.barTintColor;
            }
            if (bgColor) {
        ...
        [self.navigationController.navigationBar addSubview:self.weekView];
    }
...
//    NSLog(@"%d - %d", indexPath.section, indexPath.item);
    ...
    if (cellDateComponents.month == firstOfMonthsComponents.month) {
        ...
        cell.enabled = [self isEnabledDate:cellDate];
    } else {
    ...
    } else if(isToday || !cell.enabled || cell.isWeekend){
...
- (UICollectionReusableView *)collectionView:(UICollectionView *)collectionView viewForSupplementaryElementOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath
{
    if (kind == UICollectionElementKindSectionHeader) {
        ...
        NSDate *firstOfMonth = [self firstOfMonthForSection:indexPath.section];
        NSDate *cellDate = [self dateForCellAtIndexPath:indexPath];
        NSInteger val = [self.calendar components:NSDayCalendarUnit fromDate:cellDate toDate:firstOfMonth options:0].day;
        CGFloat width = CGRectGetWidth(self.collectionView.bounds) / self.daysPerWeek;
        CGFloat offset = width * val;
        CGRect frame = headerView.titleLabel.frame;
        frame.origin.x = offset;
        frame.size.width = width;
        headerView.titleLabel.frame = frame;
        headerView.titleLabel.text = [self.headerDateFormatter stringFromDate:firstOfMonth].uppercaseString;
        return headerView;
    }
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout *)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    return CGSizeMake(itemWidth, 70.f);
}
...
- (NSDate *)firstOfMonthForSection:(NSInteger)section
{
    ...
    if (!_cacheDateForSection[key]) {
        NSDateComponents *offset = [NSDateComponents new];
        offset.month = section;
        _cacheDateForSection[key] = [self.calendar dateByAddingComponents:offset toDate:self.firstDateMonth options:0];
    }
    return _cacheDateForSection[key];
}
...
- (NSDate *)dateForCellAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (!_cacheDateForCell[key]) {
        ...
        _cacheDateForCell[key] = [self.calendar dateByAddingComponents:dateComponents toDate:firstOfMonth options:0];
    }
    return _cacheDateForCell[key];
}
